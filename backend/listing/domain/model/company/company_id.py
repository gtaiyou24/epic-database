...
class CompanyId:
    class Type(Enum):
        ...
        def make(self, value: str) -> CompanyId:
            return CompanyId(value, self)

    value: str
    type: Type
    other_id: CompanyId | None = None

    def __init__(self, value: str, type: Type, other_id: CompanyId | None = None):
    ...
    def __eq__(self, other: CompanyId):
        for type in CompanyId.Type:
    ...
    @staticmethod
    def of(value: str) -> CompanyId:
        for type in CompanyId.Type:
            if type.match(value):
                return type.make(value)
        raise ValueError('無効な企業IDです')

    def type_of(self, type: Type) -> CompanyId | None:
        if self.type == type:
            return self
        if self.other_id:
    ...
    def set_other_id(self, other_id: CompanyId) -> CompanyId:
        ...
        return CompanyId(self.value, self.type, other_id)
